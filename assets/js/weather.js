...
const loadDayForecastData = (e, city = 0) => {
  ...
  for (let i = 0; i < dayKey.length; i++) {
    document.getElementById(`${lateId[i]}`).innerHTML =
      weather_data[city]["forecast_today"][0][`${dayKey[i]}`];
    document.getElementById(`${nightId[i]}`).innerHTML =
      weather_data[city]["forecast_today"][1][`${dayKey[i]}`];
  }
};
...
const loadWeekForecastData = (e, city = 0) => {
  ...
  for (let i = 0; i < len; i++) {
    let text = weather_data[city]["forecast_week"][i]["text"];
    let date = weather_data[city]["forecast_week"][i]["date"];
    let icon = weather_data[city]["forecast_week"][i]["icon"];
    let min = weather_data[city]["forecast_week"][i]["temperature"]["min"];
    let max = weather_data[city]["forecast_week"][i]["temperature"]["max"];
...
loadbutton.addEventListener("click", (e) => {
  loadWeekForecastData(null, city);
});
